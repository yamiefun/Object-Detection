...
def img_bbox_data_constructor(mat_file):
    ...
    bbox_df = pd.DataFrame([], columns=['height', 'img_name', 'label', 'left',
                                        'top', 'width'])
    for j in range(f['/digitStruct/bbox'].shape[0]):
        ...
        bbox_df = pd.concat([bbox_df, pd.DataFrame.from_dict(
    ...
    for i in range(1, 32403):
        ...
        with open(file_name, "w") as f:
            ...
            for j in range(len(labels)):
                ...
                x_center = ((labels.loc[j]['left']+labels.loc[j]['right'])/2)
                x_center /= img_w
                y_center = ((labels.loc[j]['bottom']+labels.loc[j]['top'])/2)
                y_center /= img_h
...
if __name__ == "__main__":
    ...
    img_bbox_data = img_bbox_data_constructor(os.path.join(img_folder,
                                                           mat_file_name))
